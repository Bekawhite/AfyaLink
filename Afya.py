...
class Authentication:
    def __init__(self):
        self.credentials = {
            'usernames': {
                ...
                'investor': {
                    'password': self._hash('investor2024'),
                    'email': 'investor@afyalink.go.ke',
                    'role': 'Investor',
    ...
    def setup_auth_ui(self):
        ...
        if not st.session_state.get('authenticated'):
            ...
            with col2:
                if st.button("Investor Demo", use_container_width=True):
                    user = self.authenticate('investor', 'investor2024')
                    if user:
                        st.session_state.user = user
                        st.session_state.authenticated = True
                        st.rerun()
...
class PresentationEnhancer:
    """Adds presentation-ready animations and visualizations"""
    ...
    @staticmethod
    def impact_chart(db):
        """Generate impact metrics for investor presentation"""
        patients = db.get_all_patients()
        ambulances = db.get_all_ambulances()
        
        total_transfers = len(patients)
        if total_transfers == 0:
            total_transfers = 156  # Demo data
        
        # Calculate estimated lives impacted (assuming each transfer affects patient + family)
        lives_impacted = total_transfers * 5
        
        # Calculate cost savings
        total_savings = sum(a.cost_savings for a in ambulances) if ambulances else 750000
        
        # Calculate response time improvement (demo calculation)
        avg_response = 18.4  # minutes
        national_avg = 45  # minutes
        improvement = ((national_avg - avg_response) / national_avg) * 100
        
        return {
            'transfers_completed': total_transfers,
            'lives_impacted': lives_impacted,
            'cost_savings_ksh': total_savings,
            'response_improvement': improvement,
            'active_ambulances': len([a for a in ambulances if a.status == 'Available']) if ambulances else 15,
            'facilities_connected': 40
        }

# ─────────────────────────────────────────────────────────────────────────────
# INVESTOR PRESENTATION UI
# ─────────────────────────────────────────────────────────────────────────────
class InvestorPresentationUI:
    """Professional presentation mode for investors and professors"""
    
    def __init__(self, db, analytics):
        self.db = db
        self.analytics = analytics
        self.enhancer = PresentationEnhancer()
    
    def display(self):
        self.enhancer.hero_section()
        ...
        # Key Impact Metrics
        impact = self.enhancer.impact_chart(self.db)
        ...
        with col1:
            ...
            <div style='background:linear-gradient(135deg,#ffebee,#ffcdd2);border-radius:16px;padding:1.5rem;height:100%;border:1px solid #ef9a9a;'>
                ...
                <ul style='color:#37474f;line-height:1.8;'>
                    ...
                    <li><strong>Inefficient ambulance dispatch</strong> leading to fuel waste</li>
                    <li><strong>Limited data</strong> for health system optimization</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            <div style='background:linear-gradient(135deg,#e8f5e9,#c8e6c9);border-radius:16px;padding:1.5rem;height:100%;border:1px solid #a5d6a7;'>
                ...
                <ul style='color:#37474f;line-height:1.8;'>
                    ...
                    <li><strong>Cost intelligence</strong> for operational optimization</li>
                    <li><strong>Scalable architecture</strong> ready for national deployment</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
        ...
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("Total Population", "54M+", "Kenya")
        with col2:
            st.metric("Public Health Facilities", "6,600+", "Nationwide")
        with col3:
            st.metric("Counties", "47", "Target Rollout")
        with col4:
            st.metric("Ambulance Fleet", "~2,000", "Potential Market")
        ...
        with col2:
            ...
            <div style='background:#ffffff;border-radius:16px;padding:1.5rem;border:1px solid #e0e0e0;'>
                ...
                <ul style='color:#37474f;'>
                    ...
                    <li>Private Hospital Networks</li>
                    <li>NGOs and Emergency Services</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col2:
            if st.button("📊 Request Full Investment Deck", use_container_width=True, type="primary"):
                st.success("Demo request sent! In production, this would email the investor relations team.")
                st.balloons()
...
class NotificationService:
    ...
    def send_notification(self, recipient, message, ntype):
        ...
        st.toast(f"{labels.get(ntype, '📧')} → {recipient}", icon="✅")
        return True
...
class AnalyticsService:
    ...
    def get_kpis(self):
        ...
        total      = len(patients) if patients else 156  # Demo data
...
class DashboardUI:
    ...
    def _facility_map(self):
        st.markdown("##### 🗺️ Kisumu Network — 40 Facilities")
    ...
    def _create_referral_form(self):
        ...
        with st.form("referral_form", clear_on_submit=True):
            ...
            if from_hosp == to_hosp:
                st.warning("⚠️ Referring and receiving facilities must be different.")
                ...
                with st.expander(f"🚑 {p.name} — {p.condition} ({p.status})", expanded=True):
                    if ambulance and p.trip_distance:
                        ...
                        c4.metric("Fuel Level",f"{ambulance.fuel_level:.1f}%",
                                  "🟢" if ambulance.fuel_level>50 else "🟡" if ambulance.fuel_level>20 else "🔴")
...
class CostManagementUI:
    ...
    def _budget(self):
        ...
        c3.metric("Budget Status",
                  "✅ Within Budget" if net<=budget else "⚠️ Over Budget",
...
class CommunicationUI:
    ...
    def _all_messages(self):
        ...
        with c1:
            ftype = st.selectbox("Filter", ["All","🤖 System Auto","👨‍⚕️ Staff","🚑 Driver"])
        with c2:
        ...
        elif ftype == "👨‍⚕️ Staff":   comms = [c for c in comms if c.sender not in ['AfyaLink System','Driver']]
        if search:
    ...
    def _templates(self):
        templates = {
            ...
            "📍 Status Updates": {
                ...
                "Traffic Delay":     "⏱️ DELAY: Traffic on route. New ETA {X} minutes. Patient stable.",
            ...
            "📊 Clinical Updates": {
                ...
                "Condition Change":"⚠️ CONDITION CHANGE: Patient deteriorating. Prepare advanced support on arrival.",
...
class NationalNetworkUI:
    def display(self):
        ...
        st.markdown("### 🏛️ County Onboarding Status")
        c_data = []
...
class AfyaLinkApp:
    def __init__(self):
        ...
        self.investor_ui      = InvestorPresentationUI(self.db, self.analytics)
    ...
    def _landing(self):
        PresentationEnhancer.hero_section()
        ...
        st.markdown("*Use any of these demo accounts to explore the platform*")
    ...
    def _users(self):
        ...
        with c1:
            ...
            with st.form("add_user"):
                ...
                role     = st.selectbox("Role", ["Admin","Hospital Staff","Ambulance Driver","Investor"])
        ...
        with c2:
            ...
            users = [
                ...
                {"Name":"Investor View","Username":"investor","Role":"Investor","Facility":"Ministry of Health","Status":"✅ Active"},
            ]
